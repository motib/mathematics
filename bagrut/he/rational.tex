...
אני לא ממש אוהב את השאלה כי היא מחייבת חישובים רבים עם מחשבון!
...
f''(x)&=& 2 + 4\sin 2x\,.
\end{equationarray*}
השאלה מבקשת את נקודות הקיצון של הנגזרת הראשונה ונקבל אותם על ידי חישוב הנקודות בהן הנגזרת השנייה מתאפסת:
...
\draw[thick,dashed] (O) -- (-75:2) node[below right] {$-5\pi/12$};
\draw[very thick,dotted] (O) -- (-30:2) node[below right] {$-\pi/6$};
...
כדי לבדוק אם הנקודות הקיצון הן מינימום או מקסימום, נחשב את הנגזרת השנייה של 
...
\end{equationarray*}
מכאן שהשיפוע הוא מקסימלי כאשר 
$x=-\frac{5\pi}{12}$
ומינימלי כאשר
$x=-\frac{\pi}{12}$.

חשוב לשים לב שהשאלה מבקשת את ערכי הקיצון של 
\textbf{השיפועים} 
...
ו-%
$\left(-\disfrac{5\pi}{12},1.21\right)$.
...
הנגזרת השנייה היא:
\[
f''\left(-\frac{5.5\pi}{12}\right)= 2 + 4\sin \left(2\cdot -\frac{5.5\pi}{12}\right)=3.04>0\,,
\]
...
הנגזרת השנייה היא:
\[
f''\left(-\frac{\pi}{24}\right)= 2 + 4\sin \left(2\cdot -\frac{\pi}{24}\right)=3.04>0\,,
\]
והפונקציה קעורה כפלי מעלה ב-%
$-\frac{\pi}{12}<x<0$.
...
\vspace{-2ex}
שאלה מאוד ארוכה. קצת מפחיד!

\textbf{סעיף א}

נקודות הקיצון הן הנקודות בהן הנגזרת הראשונה מתאפסת, ונתון ההיא מתאפסת ב-%
$\frac{\pi}{3}$
)לפחות(:
...
\]
המשוואה נראית די מסובכת אלא אם נשים לב שמתנה: ניתן לפשט אותו על ידי שימוש בנוסחה עבור
$\sin (\theta+\theta)$:
\erh{12pt}
\begin{equationarray*}{l}
-8b\sin x\cos x - 4b\sin 4x=\\
-4b(2\sin x \cos x + \sin (2x+2x))=\\
...
\item
אני אוהב להשתמש בתנועות ידיים כדי "לראות שיפועים" ולקבוע אם פונקציה עולה או יורדת וכן אם נקודות קיצון היא מקסימום או מינימים. אני מזיז כף יד שטוחה לאורך הפונקציה בכיוון משליליים לחיוביים בציר ה-%
$x$.
אם היד מוטה למטה הפונקציה יורדת, והיא עולה אם היד מוטה למעלה. אם היד עוברת מכיוון למטה לכיוון למעלה, השיפוע, הנגזרת, יורדת ואז עולה, כך שנקודת הקיצון היא מינימים.
...
\item
אל תקצרו בחישובים. קשה לספור את מספר הפעמים שטעיתי כי השמטתי סימן מינוס בטעות. לוקח מעט מאוד זמן לרשום שורה נוספת, לעומת הזמן הדרוש לחפש שגיאה בחישוב מקוצר.

\item
בבעיות עם פונקציות טריגונומטריות, ציירו תרשים של מעגל היחידה וסמנו עליו את תחום ההגדרה. מתרשים יעזור בקביעת סימני הפונקציות ובערכי הפונקציות כאשר מוסיפים או מחסירים כפולות רציונליות של 
$\pi$.
\end{itemize}
