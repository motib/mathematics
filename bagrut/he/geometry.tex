...
\fill (D) node[above,yshift=6pt] {$D$} circle(1.5pt);
...
אם במשולשים ישר זווית
$\triangle ADE,\triangle CDE$
יש זווית אחת של
$45^\circ$,
גם הזוויות החדות האחרות
$\angle DAE,\angle DCE$
שוות
$45^\circ$,
...
$\angle MGA\sim \angle DGL$
קודקודיות. גישה זו לא הצליחה כי לא מצאתי דרך לבטא את הקשר בין
$AD$
לבין 
$AG, GD$.
לבסוף שמתי לב למשולשים
$\triangle LDA,\triangle LMA$
יתר משותף, והמשולש
$\triangle LMA$
שווה שוקיים כי הצלעות הם רדיוסים. ממשפט פיתגורס:
\begin{eqnarray*}
AM^2+ML^2&=&AL^2\\
AM&=&ML\\
2AM^2&=&AL^2\\
...
מעויין הוא מקבילית עם צלעות שווים. כמקבילית, משפט 
$28$
"במקבילית האלכסונים חוצים זה את זה" טופס, וסימנו את אורכי האלכסון
$AC$
ב-%
$b$.
ביחד עם משפט
...
$AC$.
$G$
היא נקודת החיתוך של שני אנכים אמצעיים, והטענה נובעת ממשפט
$54$
...
\path (B) -- node[right] {$a$} (F) -- node[right] {$a$} (C);
...
הוא אנך אמצעי ל-%
$BC$.
בציור למטה הדגשנו את המשולשים שיש להוכיח שהם דומים. אפשר לראות שהזווית
$\alpha$
היא משותפת למשולשים ישר זווית, כך ש-%
$\triangle BKC\sim \triangle MFC$.
הזווית
$\beta$
משותפת למשולשים ישר זווית, כך ש-%
...
נוכיח שהמשולש
$\triangle GAD$
שווה שוקיים על ידי הוכחה ש-%
$\angle AGD=\beta$
כי המשלים שלה
$\angle CGA=\beta'$.
נזכור ש-%
$ABCG$
דלתון והזוויות הצדדיות שלו שוות. רשימת המשפטים לא כוללתת משפט זה אז נצטרך להוכיח אותו. דלתון הוא ריבוע עם שני זוגות של צלעות סמוכות שוות. במילים אחרות, דלתון מורכב משני משולשים שווה שוקיים המוצמדים בבסיסיהם: 
\[
\angle ABC = \angle ABG + \angle GBC = \angle AGB + \angle BGC = \angle AGC\,.
\]
מכאן, 
\[
\angle AGC = 180-AGC=180-\angle ABC=180-(180-\beta)=\beta\,.
\]
...
ולפי זוויות משלימות:
\[
\angle KBA=180-\angle CBA = 180-\angle CGA=\angle AGD=\angle GDA\,,
\]
כי
$\triangle GAD$
שווה שוקיים.
...
\fill (K) node[below left] {$K$} circle(1.5pt);
\path[name intersections={of=abd and lc,by={dummy,G}}];
\fill (G) node[above] {$G$} node[below right,xshift=-1pt,yshift=-6pt] {$\beta$} node[below left,xshift=1pt,yshift=0pt] {$\beta'$} circle(1.5pt);
\draw (A) -- (G);
\draw (C) -- (B);
\draw[very thick] (A) -- (B);
\draw[very thick] (A) -- (D);
...
\draw[very thick] (A) -- (K);
\draw[very thick] (D) -- (C);
\node at ($(C)+(-8mm,-5mm)$) {$\alpha$};
\draw[->] ($(C)+(-4mm,-4mm)$) -- +(14pt,0);
\draw[->] ($(C)+(-4mm,-6mm)$) -- +(22pt,0);
\node at ($(A)+(-8mm,-2mm)$) {$\alpha$};
\draw[->] ($(A)+(-7mm,0mm)$) -- +(7pt,11pt);
\end{tikzpicture}
\end{center}
\vspace*{-25mm}
דרך אחרת להוכיח ש-%
