...
כי הם נשענים על אותן קשתות. התרשים נראה דחוס אבל זה בגלל שציירתי לפי הזוויות המחושבות במהלך הפתרון.
\begin{center}
\selectlanguage{english}
\begin{tikzpicture}[scale=1.3]
\clip(-8mm,-5mm) rectangle +(12,2.8);
\coordinate (B) at (0,0);
...
\fill (D) node[above,yshift=-2pt] {$D$} node[left,xshift=-18pt,yshift=1pt] {$3\beta$} node[below,yshift=-4pt] {$180\!-\!3\beta$} circle(1.5pt);
...
$b$
ונקבל
$r=a\cos 2\beta\tan\beta=a\cos 20\tan 10=0.1657a$.
...
\fill (B) node[right] {$B$} node[above left,xshift=-8pt] {$60$} node[above left,xshift=2pt,yshift=24pt] {$30$} circle(1.5pt);
...
\path (C) -- node[above,xshift=-2pt] {$4x$} (M) -- node[above,xshift=-2pt] {$\sqrt{3}$} (A);
\end{tikzpicture}
\end{center}
...
$(1)$
נשתמש בחוק הסינוסים ונקבל שתי משוואות עם הנעלמים
$x,\alpha$.
נפתור את המשוואות כדי לקבל משוואה אחת עם הנעלם
$\alpha$:
\erh{12pt}
\begin{equationarray*}{rcl}
...
\frac{1}{2\sqrt{3}\sin\alpha}&=&\frac{\sqrt{3}}{4\cdot 2\cos \alpha}\,.\end{equationarray*}
מכאן ש-%
$\angle BAM=\alpha = 53.13$
$\tan \alpha = \displaystyle\frac{4}{3},$.

לא לשכוח לרשום גם את הערך של 
$\angle BCM=90\!-\!\alpha=36.87$!
...
$\triangle ABM$:
\[
2R_1=\frac{8}{\sin\alpha},\quad R_1=5\,.
\]
עובר
$\triangle CMB$:
\[
2R_2=\frac{8}{\sin(90\!-\!\alpha)},\quad R_2=\frac{20}{3}\,.
\]

\textbf{סעיף ב}

$(1)$
מהתרשים ניתן לראות ש-%
$O_1M=O_1B$
...
כי הם רדיוסים של המעגל החוסם את
$\triangle CBM$.
לפי ההגדרה מרובע עם שני זוגות של צלעות שווים הוא דלתון.
\begin{center}
\selectlanguage{english}
\begin{tikzpicture}%[scale=1.1]
...
\draw[thick] ($(A)+(12mm,0)$) arc[start angle=0,end angle=37,radius=12mm];
\node[right,xshift=34pt,yshift=6pt] at (A) {$\beta\!-\!\alpha$};
\node[above right,xshift=34pt,yshift=14pt] at (A) {$\alpha$};
\node[above right,xshift=28pt,yshift=28pt] at (A) {$\alpha$};
\node at (0,2) {$\angle OAF=\beta$};
\end{tikzpicture}
\end{center}
...
\draw[rotate=90] (H) rectangle +(7pt,7pt);
