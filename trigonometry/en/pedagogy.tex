...
\draw[step=10mm,white!80!black,ultra thin] (-3,-2) grid (8,2);
\draw (-3,0) -- (8,0);
\draw (0,-2) -- (0,2);
...
%  \draw[white!70!black,thin] (\x,-3) -- (\x,3);
  \node at (\x,-.2) {$\rad$};
  \node at (\x,-.8) {$\deg$};
}
\foreach \y/\s in {-3/,-2/-1,-1/-.5,1/.5,2/1,3/} {
%  \draw[white!70!black,thin] (-3.14,\y) -- (7.85,\y);
  \node at (.3,\y) {$\s$};
}
\draw (-3.14,0) -- (7.85,0);
\draw (0,-2) -- (0,2);
...
\begin{center}
\begin{tikzpicture}[scale=1.2]
\draw[step=10mm,white!80!black,ultra thin] (-2,-2) grid (7,2);
\draw (-2,0) -- (7,0);
\draw (0,-2) -- (0,2);
...
\node[red] at (2.6,1.4) {$\sin x_{radians}$};
\node[blue] at (4.4,1.4) {$\sin' x_{radians}$};
\end{tikzpicture}
...
\draw[step=10mm,white!80!black,ultra thin] (-2,-2) grid (6,2);
\draw (-2,0) -- (6,0);
\draw (0,-2) -- (0,2);
\foreach \x/\deg in {-2/-400,-1/-200,0/0,1/200,2/400,3/600,4/800,5/1000,6/1200}
  \node at (\x,-.3) {$\deg$};
\foreach \y in {-1,-.5,0,.5,1}
  \node at ($(-.3,0)+(0,2*\y)$) {$\y$};
...
\node[red] at (3.2,1.6) {$\sin x_{degrees}$};
\node[blue] at (1,.3) {$\sin' x_{degrees}$};
\end{tikzpicture}
...
\draw[step=10mm,white!80!black,ultra thin] (-2,-2) grid (4,2);
\draw (-2,0) -- (4,0);
\draw (0,-2) -- (0,2);
...
Consider now the function that maps of the length of the thread $x$ to the $y$ coordinate of $P$. In the figure, we see that $x=3.3$ is mapped into $y=0.7$.
...
A fruitful exercise for students is to explore the winding operation for various polygons (see \geoproject{g.winding}).
...
\begin{tikzpicture}[scale=1.5]
\begin{scope}
\draw[step=10mm,white!80!black,ultra thin] (-2,-2) grid (2,2);
\draw (-2,0) -- (2,0);
\draw (0,-2) -- (0,2);
...
\draw[xstep=1.57,ystep=10mm,white!80!black,ultra thin] (-1.57,-2) grid (3.14,2);
\draw (-1.57,0) -- (3.14,0);
\draw (0,-2) -- (0,2);
...
\caption{Functional definition of the function $s(x)$}\label{fig.function-s}
\end{center}
\end{figure}

Figure~\ref{fig.function-s} shows the definition of the function $s(x)$ in a split window: the right window shows the independent variable (point $X$) and the dependent variable (point $S$), the value of the function. The left window shows the geometric construction that defines the function.\footnote{In Geogebra, constructions such as these can be done in one coordinate system or two. See \url{https://www.geogebra.org/m/wzsmbz9c}) and \url{https://www.geogebra.org/m/g3rdcmat}.}
...
The graphical representation of the function for all real values can be obtained by tracing the points $P$ as you drag the point $X$ left and right (Figure~\ref{fig.trace-of-the-function}). See \geoproject{g.trace}.
...
\draw[xstep=1.57cm,ystep=1cm,white!80!black,ultra thin] (-1.58,-2) grid (7.86,2);
\draw (-1.57,0) -- (7.85,0);
\draw (0,-2) -- (0,2);
...
\begin{center}
\begin{tikzpicture}[scale=1.2]
\draw[step=10mm,white!80!black,ultra thin] (-3,-3) grid (3,3);
\draw[thin] (-3,0) -- (3,0);
\draw[thin] (0,-3) -- (0,3);
...
Next, the computational benefits that can be derived from the cyclicality and symmetry features that are derived from the function definition must be examined. 
...
A good exercise at this point is to identify the values $V=\{x_1,x_2,\ldots\}$ of $x$ that such that $s(x_i)=\pm s(x)$. Suppose we know that $s(\pi/6)=1/2$. We now ask for the value of $x$ for the three symmetrical points (Figure~\ref{fig.computing}). Note that each point is also reached by winding the thread around the circle in a negative direction. Finally, since the function is periodic, for example, $s(-5\pi/6)=1/2$, $V=\{x_1,x_2,\ldots\}$ is an infinite set.

\vspace*{50pt}

\section{The function $c$ maps a real number to a coordinate}

The definition of the function $s$ leads naturally to the definition of a function $c$ which maps a real number (the length of an arc on the circumference of the unit circle) to the $x$-value of the end point of the arc (Figure~\ref{fig.cosine}).
\begin{figure}[H]
\begin{center}
...
The \emph{point} $X$ (upper case) identifies a point along the $x$-axis that the length from the origin to $X$ specifies the length of the arc from $(1,0)$ around the circumference of the circle.
The \emph{variable} $x$ (lower case) is the $x$ value of the coordinates of the endpoint of this arc.
\end{quote}

In a Geogebra project, a trace of the value of $c(x)$ as $x$ is moved along the $x$-axis gives a graphical representation of the function (Figure~\ref{fig.cosine-trace}).
...
\end{scope}
\begin{scope}[xshift=4.5cm]
\draw[xstep=1.57cm,ystep=1cm,white!80!black,ultra thin] (-1.58,-2) grid (3.16,2);
\draw (-1.57,0) -- (3.16,0);
\draw (0,-2) -- (0,2);
...
By looking again at Figures~\ref{fig.symmetries}, we can compute the symmetry properties of the function $c(x)$. We leave it to the reader to justify the following equalities:
\begin{eqnarray*}
c(P_x) &=& c(P)\\
c(P_y) &=& -c(P)\\
c(P_{xy}) &=& -c(P)\,.
...
\draw[xstep=1.57,ystep=1,white!80!black,ultra thin] (-1.57,-2) grid (3.14,2);
\draw (-1.57,0) -- (3.14,0);
\draw (0,-2) -- (0,2);
...
Similarly, the cotangent function is defined as the intersection of the line through $O$ and $P$ with the horizontal line $y = 1$. See \geoproject{g.cotangent}.
...
\begin{tikzpicture}
\begin{scope}[scale=.6]
\draw[step=2cm,white!80!black,ultra thin] (-6.1,-6) grid (6,6);
\draw[thin] (-6,0) -- (6,0);
\draw[thin] (0,-6) -- (0,6);
...
\draw[xstep=1.57,ystep=1,white!80!black,ultra thin] (-1.57,-3) grid (4.71,3);
\draw (-1.57,0) -- (4.71,0);
\draw (1.57,-3) -- (1.57,3);
...
\caption{The cotangent function}\label{fig.15}
\end{center}
\end{figure}
...
If the geometric definition is used, it would also be necessary to discuss other geometric definitions such as $\tan x = \disfrac{\sin x}{\cos x}$ and $\sin^2 x+\cos^2 x =1$.
Of course, this involves introducing the triangular approach at this point. 
Furthermore, it will be necessary to take the signs into account (Figure~\ref{fig.signs}). See \geoproject{g.signs}.
...
\draw[thin] (0,-6) -- (0,6);
  ...
  \draw[rotate=-90] (Ps) rectangle +(8pt,8pt);
  \path [name intersections = {of = side and tangent, by = {T}}];
