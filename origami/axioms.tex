...
This document develops the mathematical formulas of the seven origami axioms together with numerical examples. The Wikipedia article \cite{hh} gives the axioms, as well as parametric equations for the first five axioms. Here only secondary-school analytic geometry is used; in particular, equations of lines are given in the familiar slope-intercept form $y=mx+b$.

Appendix~A contains links to GeoGebra projects that use the data of the examples.

Each section states an axiom which is illustrated by a diagram. Then the equation of the fold is derived followed by a numerical example.

In the diagrams, given lines are solid, folds are dashed, auxiliary lines are dotted, and dotted arrows are used as an indication of the direction of folding the paper.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section*{Definitions}

Each axiom states that a \emph{fold} (possibly more than one) exists that will place given points and lines onto points and lines, such that certain properties hold. The term fold comes from the origami operation of folding a piece of paper, but here it is used to refer the the line created by folding the paper.

Formal definitions are given in \cite[Chapter~10, Paperfolding]{martin}. The reader should be aware that, \emph{by definition}, folds result in \emph{reflections}. Given a point $p$, its reflection around a line $l$ results in a point $p'$, such that $l$ is the perpendicular bisector of the line segment $\overline{pp'}$:
...
\draw[thick] (m1) -- +(120:6pt);
\draw[thick] (m1) -- +(-60:6pt);
\draw[thick] (m2) -- +(120:6pt);
\draw[thick] (m2) -- +(-60:6pt);

\draw[thick] (P1) -- (P1P);
\draw[very thick,dashed] (4.7,1.6) -- node[very near end,right,xshift=2pt] {$l$} (3.5,4);
...
$p_i=(x_i,y_i)$, the point of intersection of the two lines:
\vspace{-2ex}
\begin{form}{2}
...
\draw[thick] (L1a) -- node[near end,below right,yshift=-2pt] {$l_1$} (L1b);
...
For a given pair of points and a line, there may be zero, one or two solutions.

\textbf{Derivation of the equations of the reflections}

Let $l$ be a fold through $p_2$ and $p_1'$ be the reflection of $p_1$ around $l$. The length of the line segment $p_1p_2$ equals the length of the line segment $p_2p_1'$. The locus of points at distance $p_1p_2$ \emph{from} $p_2$ is the circle centered at $p_2$ whose radius is the length of $\overline{p_1p_2}$. The intersections of this circle with the line $l_1$ give the possible points $p_1'$.

Let $l_1$ be $y=m_x + b_1$ and let $p_1=(x_1,y_1)$, $p_2=(x_2,y_2)$. The equation of the circle centered at $p_2$ with radius the length of $p_1p_2$ is:
\begin{form}{1.5}
...
\[
p_1'=\left(-\disfrac{2}{5},\disfrac{16}{5}\right) \approx (-0.4,3.2)\,,\quad p_1''=(6,0)\,.
\]

\textbf{Derivation of the equations of the folds}

The folds will be the perpendicular bisectors of $p_1p_1'$ and $p_1p_1''$. The equation of a perpendicular bisector is given by Equation~\ref{eq.midpoint1} repeated here with for $p_1'$:
\begin{equation}
...
For a given pair of points and pair of lines, there may be zero, one, two or three solutions.
...
A fold that places $p_i$ onto $l_i$ is a line such that the distance from $p_i$ to the line is equal to the distance from $l_i$ to the line. The locus of points that are equidistant from $p_i$ and $l_i$ is a parabola with focus $p_i$ and directrix $l_i$. A fold is any line tangent to that parabola. A detailed justification of this claim is given in Appendix~C.

For a fold to simultaneously place $p_1$ onto $l_1$ and $p_2$ onto $l_2$, it must be a tangent common to the two parabolas.

The formula for an arbitrary parabola is quite complex,
...
Let $(0,f)$ be the focus of a parabola with directrix $y=d$. Define $p=f-d$, the signed length of the line segment between the focus and the directrix.\footnote{We have been using the notation $p_i$ for points; the use of $p$ here might be confusing but it is the standard notation. The formal name for $p$ is one-half the \emph{latus rectum}.} If the vertex\footnote{The vertex of a parabola whose axis of symmetry is parallel to the $y$-axis is its minimum or maximum value.} of the parabola is on the $x$-axis, the equation of the parabola is $y=\disfrac{x^2}{2p}$. To move the parabola up or down the $y$-axis so that its vertex is at $(0,h)$, add $h$ to the equation of the parabola: $y=\disfrac{x^2}{2p}+h$. Define $a=2ph$ so that the equation of the parabola is:
\begin{form}{1.5}
y=\disfrac{x^2}{2p}+\disfrac{a}{2p}\\
x^2-2py+a=0\,.
\end{form}
Substitute the equation of a line $y=mx+b$ into the equation for the parabola to obtain an equation for their points of intersection:
\begin{form}{1.5}
x^2-2p(mx+b)+a=0\\
x^2+(-2mp)x+(-2pb+a)=0\,.
\end{form}
The line will be tangent to the parabola iff this quadratic equation has \emph{exactly one} solution iff the discriminant is zero:
\[
...
This is the equation with variable $m$ for the tangents to the parabola. There are an infinite number of tangents because for each $m$, there is an $b$ that makes the line a tangent by moving it up or down.
...
2m^2-b-4=0\,,
\end{form}
tare $m=\pm\sqrt{\disfrac{7}{3}}\approx \pm 1.53$ and $b=\disfrac{2}{3}$. There are two common tangents that are folds:
\[
...
m=3.73\,, \;m=-1\,, \; m=0.27\,.
\]
Choosing $m=0.27$, the equation of the fold is:
\begin{form}{1.4}
b=3-m^2=2.93\\
y=0.27x+2.93\,.
\end{form}
...
y_t=m_tx_t+b_t\,.
\end{form}
The reflection $p_1'$ is easy to dervie because the intersection $p_t$ is the midpoint between $p_1$ and its reflection $p_1'$:
\begin{form}{2}
...
Since the fold $l$ is perpendicular to $l_2$ then the line $l_p$ containing $\overline{p_1p_1'}$ is perpendicular to $l$ and thus parallel to $l_2$. This line passes through $p_1$ so its equation is:
\begin{form}{1.5}
y_1=m_2x_1+b_p\\
y=m_2x+(y_1-m_2x_1)\,.
\end{form}
$p_1'=(x_1',y_1')$, the reflection  of $p_1$ around the fold $l$, is the intersection of $l_1$ with $l_p$:
\begin{form}{1.5}
...
\section{Derivation of the trigonometric identities}

The trigonometric identifies used in axiom 3 can be derived from the identifies for the sine and cosine of the sum of two angles:
...
We use this formula for $\theta_1=\theta_2=\theta/2$ and obtain a quadratic equation in $\tan\theta/2$:
\begin{form}{2}
...
\[
y=ax^2+by+c\,.\footnote{These are parabolas whose axes of symmetry are parallel to the $y$-axis. To simplify the presentation of the axiom 6, it is limited to parabolas whose axes of symmetry are parallel to the $x$- or $y$-axis.}
\]
...
The following diagram shows the focus, the large point at $p=(0,f)$, and the directrix, the thick line whose equation is $y=-f$. The resulting parabola is shown as a dotted curve. Its vertex is at the origin of the axes.
%\begin{figure}[H]
\begin{center}
\begin{tikzpicture}[scale=1.2]
\draw (-6,0) -- node[very near start,below,xshift=-30pt] {$x$}(6,0);
\draw (0,-3) -- node[very near start,right,yshift=-20pt] {$y$}(0,4.5);
...
\fill (FP) circle (1.5pt) node[above left] {$p'$};
...
We have selected five points $p_i$, $i=1,2,3,4,5$, on the parabola. Each point $p_i$ is at a distance of $a_0$ both from the focus and the directrix.

Consider the point $p'$ that is the intersection of the perpendicular line from $p_1$ to the directrix. Since it is on the parabola $\overline{p'p_1}=\overline{p_1p}=a_1$. We claim that the tangent $l$ at $p_1$ (dashed line) is a fold that reflects $p$ onto $p'$.

We have to prove the $l$ is the perpendicular bisector of $\overline{pp'}$. Let us extract a simplified diagram:
\begin{center}
\begin{tikzpicture}[scale=1.2]
\draw[very thick] (-6,0) -- node[very near start,below,xshift=-30pt] {$x$}(1,0);
\draw[very thick] (0,-3) -- node[very near start,right,yshift=-20pt] {$y$}(0,3.5);
\draw[very thick] (-6,-2) -- (1,-2);
\coordinate (F) at (0,2);
\fill (F) circle (3pt) node[above right] {$p$};
\fill (0,0) circle (1.5pt) node[below right] {$p_2$};
\fill (FP) circle (1.5pt) node[above left] {$p'$};
\coordinate (F4) at (-5,3.125);
\fill (F4) circle (1.5pt) node[above right] {$p_1$};
...
\draw[rotate=36] (H) rectangle +(10pt,10pt);
\path (FP) -- node[above,yshift=2pt] {$b?$} (H) -- node[above,yshift=2pt] {$b?$} (F);
\draw[thick] (0,0) rectangle +(8pt,8pt);
\draw[thick] (0,-2) rectangle +(8pt,8pt);
\draw[thick] (-5,0) rectangle +(8pt,8pt);
\draw[thick] (-5,-2) rectangle +(8pt,8pt);
\fill (-5,0) circle (1.5pt) node[below left] {$q$};
\fill (-2.5,0) circle (1.5pt) node[below,yshift=-4pt] {$r$};
\path (-5,0) -- node[left] {$a_2$} (-5,-2);
\path (F4) -- node[left,xshift=-2pt] {$a$} (-2.5,0);
\end{tikzpicture}
\end{center}
Since the directrix is parallel to the $x$-axis, the focus is on the $y$-axis and $\overline{p_1p'}$ is perpendicular to the directrix, we see that $\angle p'qr, \angle pp_2r$ are right angles, and $\overline{qp'}=\overline{pp_2}=a_2$.
Since $\angle qrp', \angle p_2r$ are vertical angles they are equal, and $\triangle qrp'\cong \triangle p_2r$.
Therefore, $\overline{p'r}=\overline{rp}$, $\overline{pp_1}        = \overline{p_1p'}=a_1$, $\overline{p_1r}$ is common to $\triangle p_1rp$ and $\triangle p_1rp'$, so the triangles are congruent.
It follows that $l$ is the perpendicular bisector of $pp'$.
