...
Section~\ref{s.method} presents the full specification of Lill's method. Some of the description may be difficult to understand, but will be clarified when demonstrated by additional examples in Sections~\ref{s.negative}--\ref{s.noninteger}. Since Lill's method can find a real root of any cubic polynomial, it can be used to trisect an angle. Since it can find $\sqrt[3]{2}$ as a root of $x^3-2$, it can double a cube as shown in Section~\ref{s.cube}.
...
\item Notes:
\begin{itemize}
\item ``Intersects $a_i$'' includes ``intersects the line segment $a_i$ or any extension of it'' or ``intersects the line that contains the line segment $a_i$''.
