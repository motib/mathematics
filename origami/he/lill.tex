...
בנה מסלול המורכב מארבע קטעי קו
$\{a_3,a_2,a_1,a_0\}$
באורכים:
\[
\{a_3=1,a_2=6,a_1=11,a_0=6\}\,,
\]
החל ממרכז מערכת הצירים, תחילה בכיוון החיובי של ציר ה-%
$x$
תוך סיבוב של
$90^\circ$
נגד כיוון השעון בין הקטעים. בנה מסלול שני כדלקמן: בנה קו ממרכז הצירים בזוויות
$63.4^\circ$
יחסית לציר ה-%
$x$,
וסמן ב-%
...
פנה שמאלה 
$90^\circ$
פעם נוספת, בנה קו, ושים לב שהוא חותך את קצה המסלול הראשון הנמצא ב-%
$(-10,0)$.
...
חשב
$\tan 63.4^\circ=2$,
הטנגנס של הזווית בתחילת המסלול השני. מתקבל:
\[
p(-\tan 63.4^\circ)=(-2)^3+6(-2)^2+11(-2)+6=0\,.
\]
בשעה טובה! מצאת שורש של 
$x^3+6x^2+11x+6$,
פולינום ממעלה שלוש.
...
דוגמה זו מדגימה שיטה גרפית למציאת שורשים ממשיים של כל פולינום שגילה 
\L{Eduard Lill}
ב-%
$1867$
\L{\cite{bradford, hull-beloch, riaz}}.
נגביל כאן את הדיון לפולינומים ממעלה שלוש.

ברור שאין כאן שיטה אלגברית לחישוב שורשים של פולינומים ממעלה שלוש; למעשה, בדוגמה ווידאנו ש-%
$-2$
הוא שורש. יש עניין חדש בשיטה של
\L{Lill}
כי ניתן לממש אותה באוריגמי )פרק%
~\L{\ref{c.beloch}}(.

בסעיפים
~\L{\ref{s.multiple}--\ref{s.noroots}}
נרחיב את הדוגמה הראשונה כדי למצוא שורשים נוספים, ונראה שאם
$\alpha$
היא זווית כך ש-%
$-\tan\alpha$
\textbf{אינה}
שורש, הבנייה לא תצליח.

סעיף%
~\L{\ref{s.method}}
מביא פירוט מלא של השיטה של
\L{Lill}.
חלקים מהתיאור עלולים להיות לא ברורים, אבל יתבהרו כאשר נביא דוגמאות נוספות בסעיפים%
~\L{\ref{s.negative}--\ref{s.noninteger}}.
השיטה של
\L{Lill}
מסוגלת למצוא שורש ממשי של כל פולינום ממעלה שלוש, לכן ניתן להשתמש בה לחלק זווית לשלושה חלקים. אפשר גם למצוא את
$\sqrt[3]{2}$
כשורש של
$x^3-2$,
ולכן אפשר להכפיל קוביה כפי שנראה בסעיף%
~\L{\ref{s.cube}}.
~\L{\ref{s.proof}}
מביא הוכחה שהשיטה של
\L{Lill}
יכולה למצוא שורשים ממשיים של כל פולינום ממעלה שלוש. להוכחה עבור פולינום כלשהו מבנה דומה.

\newpage

\section{שורשים מרובים}\label{s.multiple}

נמשיך את הדוגמה. לפולינום 
$p(x)=a_3x^3+6x^2+11x+6$
שלושה שורשים
$-1,-2,-3$.
...
\]
באיור רואים שעבור כל אחת מהזוויות, המסלול השני חותך את הקצה של המסלול הראשון.
\begin{center}
\selectlanguage{english}
\begin{tikzpicture}[scale=1.15]
% Draw help lines and axes
...
\section{מסלולים שלא תואמים לשורשים}\label{s.noroots}

אולי המסלול השני חותך את המסלול הראשון עבור
\textbf{כל}
זווית, למשל, 
$56.3^\circ$.
באיור שלהלן, המסלול השני חותך את המשך קטע הקו עבור המקדם
$a_0$,
אבל לא ב-%
$(-10,0)$,
הקצה של המסלול הראשון. נסיק ש-%
$-\tan 56.3=-1.5$
\textbf{אינו}
שורש של הפולינום.
\begin{center}
...
\section{מפרט של השיטה של
\L{Lill}}\label{s.method}

כדי להבין את פרטי השיטה מומלוץ לעיין בדוגמאות בסעיפים הבאים.
\begin{itemize}
\item 
נתון פולינום שרירותי
$p(x)=a_3x^3+a_2x^2+a_1x+a_0$.
\item 
בנה את המסלול הראשון כך:
\begin{itemize}
\item
לכל מקדם
$a_3,a_2,a_1,a_0$
)בסדר זה( בנה קטע קו המתחיל במרכז הצירים
$O=(0,0)$
בכיוון החיובי של ציר ה-%
$x$.
פנה
$90^\circ$
נגד השעון בין כל קטע.
\end{itemize}
\item 
...
וסמן ב-%
$R$
את נקודה בה חותך הקו את
$a_0$.
\item
אם 
$R$
היא נקודת הקצה של המסלול הראשון, אזי
$-\tan\theta$
הוא שורש של 
$p(x)$.
\end{itemize}
...
\begin{itemize}
\item 
כאשר בונים את קטעי הקו של המסלול הראשון, אם מקדם הוא שלילי בנה את קטע הקו
\textbf{בכיוון ההפוך}.
\item
כאשר בונים את קטעי הקו של המסלול הראשון, אם מקדם הוא אפס, אל תצייר את קטע הקו, אבל המשך לפנייה הבאה של
$\pm90^\circ$.
\end{itemize}
\item הערות:
\begin{itemize}
...
"חותך הקו את
$a_i$"
 כולל המקרה "חותך את קטע הקו 
$a_i$
או כל המשך שלו" או "חותך את הקו המכיל את קטע הקו 
$a_i$".
\item
כאשר בונים את המסלול השני, בחר לפנות ימינה או שמאלה ב-%
$90^\circ$
כך שלמסלול השני תהיה נקודת חיתוך עם המסלול הראשון.
\end{itemize}
\end{itemize}
...
בסעיף%
~\L{\ref{s.ax6}}
הפעלת אקסיומה $6$ יצרה את הפולינום 
$p(x)=x^3-3x^2-3x+1$
עם מקדמים שליליים. נתחיל בבניית קטע קו באורך
$1$
לכיוון החיובי של ציר ה-%
$x$. 
אחר כך נפנה
$90^\circ$
עם הפנים למעלה, אבל המקדם שלילי, ולכן נבנה קטע קו באורך
$3$
\textbf{למטה},
הפוך מהכיוון שאנו פונים אליו. לאחר פנייה 
$90^\circ$
...
יחסית לציר ה-%
$x$.
נקודת החיתוך של הקו עם ההמשך של קטע הקו
$a_2$
היא
$(1,1)$.
נפנה שוב
$-90^\circ$
)לכיוון ימין(, נבנה קו שנקודת החיתוך שלה עם ההמשך של 
$a_1$
היא
$(5,-3)$.
נפנה שוב
...
בסעיף%
~\L{\ref{s.noninteger}}
נדון בקווים המקווקווים עם רווחים ארוכים.
...
$\pm 90^\circ$
לפני ש"בונים" אותו, כפי שניתן לראות באיור: חץ הפונה למעלה בנקודה
$(1,0)$.
אחר כך, נפנה שוב ונבנה קו באורך
$-7$,
קו באורך
$7$
אחורה לנקודה
$(8,0)$.
לבסוף, פונים שוב ובונים קו באורך
$-6$
לנקודה
$(8,6)$.
...
\section{השורש ממעלה שלוש של שניים}\label{s.cube}

כדי להכפיל קוביה עלינו למצוא
$\sqrt[3]{2}\approx 1.26$,
שורש של הפולינום ממעלה שלוש
$x^3-2$.
בבנייה של המסלול הראשון, אנו פונים פעמיים שמאלה בלי לבנות קטעי קן, כי המקדמים
$a_2$
ו-%
$a_1$
...
נגביל את הדיון לפולינומים שהמקדם הראשין הוא אפס
$p(x)=x^3+a_2x^2+a_1x+a_0$.\footnote{%
אחרת, אפשר לחלק ב-%
$a_3$
ולפולינום המקבל אותם שורשים.%
}
באיור למטה, הקטעים של המסלול הראשון מסומנים עם המקדמים ועם
$b_2,b_1,a_2-b_2,a_1-b_1$.

סכום הזוויות של משולש הוא 
$180^\circ$,
ולכן אם זווית חדה אחת היא
$\theta$, 
השנייה היא
$90^\circ-\theta$.
מכאן שהזווית מעל ל-%
