...
\end{tikzpicture}
\end{center}
יש שתי דרכים לחשב את ההסתברות. בדרך הראשונה אנו לוקחים את סכום ההסתברויות של שני האירועים ואז מחסירים את ההסתברות של האירוע המשותף כי ספרנו אותו פעמיים, פעם כחלק מהאירוע 
$I$
ופעם כחלק מהאירוע
$II$:
\[
...
\textbf{ואחר כך}
שוב מוציאים באקראי מכוונות לשימוש בעץ כדי לתאר את הבחירה הסדרתית. נסמן ב-%
\textsf{b}
...
$II$,
ונוכל לחשב את ההסתברויות להוצאת כדור אדום:
\[
\frac{5}{5+7},\; \frac{4}{4+7},\; \frac{4}{4+7},\; \frac{3}{3+7}\,.
\]
נסכם את ההסתברויות לאחר הכפלתן בהסתברות להגיע למצב:
\[
...
נבחנים מערים,
$=P(K\cap S)=p$
ההסתברות שנבחנים מקיבוצים הצליחו. ההסתברויות הנתונות הן:
\[
P(K)=0.20,\;P(M)=0.40,\;P(E)=0.40,\;P(S)=0.70\,.
\]
ניתן לחשב את שאר ההסתברויות:
\vspace{-4ex}
\[
\renewcommand{\arraystretch}{1.8}
\begin{array}{l}
P(\overline{S})=1-P(S)=0.30\\
\displaystyle P(\overline{S}/M)=P(\overline{S}\cap M) / P(M)=\frac{1}{8}\\
\displaystyle P(\overline{S}\cap M)=\frac{1}{8}P(M)=\frac{1}{8}\cdot 0.40=0.05\\
...
"$\frac{1}{8}$
\textbf{מן}
הנבחנים שהיו ממושבים נכשלו" מכוונות להסתברות מותנית, לעומת המילים "ההסתברות לבחור באקראי מבין כל הנבחנים נבחן שהיה מהעיר
\textbf{וגם}
הצליח במבחן" מכוונות לחיתוך הסתברויות. אמנם כתוב
"\textbf{מבין}
...
סעיף ב. כדאי לצייר עץ של האירועים, אבל אוותר עליו בשאלה זו כי המצב פשוט. האפשרויות לקבל שוויון הן: )א( ניצחון אחד לאנה וליעל, או )ב( תיקן בשני המשחקים. ההסתברות לתיקו היא המשלים לסכום ההסתברויות שאחת מהן תנצח. חישבנו בסעיף א את ההסתברות שיעל תנצח ולכן מקבלים משוואה בנעלם הבודד שאנה תנצח:
\begin{eqnarray*}
...
\end{array}
\]
ההסתברות לשיוון בסבב נתונה. אם אנה תנצח במשחק השני יהיה שוויון רק אם גם יעל תנצח במשחק הראשון:
\vspace{-1ex}
\[
P(\textrm{\R{אנה תנצח במשחק השני}} / \textrm{\R{תוצאת הסבב היא שוויון}})= \frac{ya}{.34}=\frac{\displaystyle\frac{1}{2}\cdot 0.3}{.34}=0.4412\,.
\]
שימו לב שלא צריכים
$2 \choose 1$
...
\end{array}
\]
החיתוך במנה שקולה ל-"בדיוק שני גברים" כי "לכל היותר שני גברים" היא 
$0,1,2$
גברים, שחישבנו בסעיף הקודם. "לכל היותר שני גברים" הוא הסכום של שלוש נוסחאות ברנולי:
\[
...
\textbf{הבחירה השנייה}
תהיה של ספרה אי-זוגית. המסלולים האלה מסומנים בכוכביות באיור. נשווה את סכום ההסתברויות של המסלולים לערך הנתון:
\[
...
\end{eqnarray*}
סעיף זה עוסק בהסתברות מותנית, אבל שני האירועים )בחירת הספרות( בלתי תלויים, ולכן אפשר לבטא את החיתוך כמכפלה והחישוב מצטמצם:
\vspace{-3ex}
\[
\renewcommand{\arraystretch}{2}
\begin{array}{c}
...
נסמן
$=b$
ההסתברות להביא אוכל מהבית. לפי נוסחת ברנולי:
\[
...
$P(R/T)=0.6$,
וגם שהסתברויות של האירועים בלתי תלויים, ולכן:
\[
...
\end{tikzpicture}
\end{center}
שבו נסתמך על העובדה שהאירועים בלתי תלויים ונקבל:
\[
...
סעיף ב. המילים "כל התושבים המשתתפים בחוג לריקודי עם, ורק הם" מכוונות להסתברות מותנית. אם ידוע שתושב משתתף בריקודי עם, ההסתברות שהוא משתתף גם בתאטרון היא:
\[
P(T/R) = \frac{P(T\cap R)}{P(R)}= \frac{P(T)\cdot P(R)}{P(R)}  = P(T) = 0.3\,,
\]
כאשר שוב השתמשנו בנתון שהאירועים בלתי תלויים. כדי לחשב "לפחות שניים" עדיף לחשב את המשלים ל-"אפס או אחד":
\[
...
סעיף א. איור~%
\L{\ref{fig.winter-2014.1}}
מראה את צבירת הנקודות של 
\textbf{אבא}
בשני הסיבובים, כאשר הקשתות מסודרות עם אבא למעלה, דני באמצע ותיקו למטה. המצבים בהם אבא צובר
\textbf{יותר}
...
\end{array}
\]
החיתוך מצטמצם כי באירוע "תיקו אחד, ניצחון אחד" חייב להיות גם האירוע "דני צבר לפחות נקודה אחת". את המכנה חישבנו בסעיף הקודם, וניתן לחשב את המנה על ידי חיבור ההסתברויות של שני מסלולים בעץ המסומנים ב-%
$\#$:
\[
...
סעיף ד. בסעיף
$2$
חישבנו את ההסתברות של האירוע בכל סיבוב, ומכאן נשאר רק לחשב לפי נוסחת ברנולי:
\[
...
\end{tikzpicture}
\selectlanguage{hebrew}
\caption{עץ ההסתברויות של המשחקים: צבירת נקודות של אבא ודני}\label{fig.winter-2014.2}
\end{center}
\end{figure}
...
"\textbf{אף אחת}".
ההסתברות שבת לא תבחר מסלול א היא ההסתברות שהיא בתחר מסלול ב.
\[
...
\textbf{אם ידוע ש-},
למשל, בבחינה של קיץ תשע"ו ב:
"חשב את ההסתברות 
$\ldots$,
\textbf{אם ידוע ש-}",
או בבחינה של קיץ תשע"ז א "%
\textbf{ידוע ש-}
$\ldots$. 
\textbf{מהי ההסתברות}
$\ldots$. "
...
\frac{P(X)}{1}=P(X)\,.
\]
מצב דומה מופיע בבחינה של קיץ תשע"ד ב.
...
\overbrace{\pm\;\pm\;\pm\;\pm\;\pm}^{2/5}\quad\quad \overbrace{+}^{1/1}\,.
\]
דרך אחרת לחשב שאלות מסוג זה הודגמה בבחינה של חורף תשע"ח, שם השתמשנו בעובדה שהטלות הקוביה הן בלתי תלויות ולכן ההסתברות של ההטלה הראשונה מצטמצמת בחישוב ההסתברות המותנית.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\item
בבחינה של קיץ תשע"ז ב הביטוי "%
