...
\node at (-3.5,.2) {$I\backslash II$};
\node at (6,.2) {$II\backslash I$};
\draw[->] (1.25,2.2) -- ++(0,-1);
\draw[->] (-3,.2) -- ++(1.7,0);
\draw[->] (5.5,.2) -- ++(-1.7,0);
\end{tikzpicture}
...
P(N\cup\overline{A})&=&P(N\backslash\overline{A}) + P(\overline{A}\backslash N) + P(N\cap\overline{A}) = 0.35+0.07+0.02=0.44\,.
\end{eqnarray*}
...
\node at (2.5,1.5) {$I\backslash II$};
\node at (1.5,2.5) {$II\backslash I$};
\draw[ultra thick] (2,2) rectangle +(1,1);
\draw[ultra thick] (1,2) rectangle +(1,1);
